...
daptin-cli:
	go get
	go build -v --ldflags "-s -X github.com/daptin/daptin-cli/fs.Version=$(TAG)" $(BUILDTAGS)
	mkdir -p `go env GOPATH`/bin/
	cp -av daptin-cli`go env GOEXE` `go env GOPATH`/bin/daptin-cli`go env GOEXE`.new
	mv -v `go env GOPATH`/bin/daptin-cli`go env GOEXE`.new `go env GOPATH`/bin/daptin-cli`go env GOEXE`

test_all:
	go install --ldflags "-s -X github.com/daptin/daptin-cli/fs.Version=$(TAG)" $(BUILDTAGS) github.com/daptin/daptin-cli/fstest/test_all
...
doc:	daptin-cli.1 MANUAL.html MANUAL.txt rcdocs commanddocs

daptin-cli.1:	MANUAL.md
	pandoc -s --from markdown --to man MANUAL.md -o daptin-cli.1
...
commanddocs: daptin-cli
	XDG_CACHE_HOME="" XDG_CONFIG_HOME="" HOME="\$$HOME" USER="\$$USER" daptin-cli gendocs docs/content/

backenddocs: daptin-cli bin/make_backend_docs.py
...
clean:
	...
	rm -f daptin-cli fs/operations/operations.test fs/sync/sync.test fs/test_all.log test.log
...
tarball:
	git archive -9 --format=tar.gz --prefix=daptin-cli-$(TAG)/ -o build/daptin-cli-$(TAG).tar.gz $(TAG)

sign_upload:
	cd build && md5sum daptin-cli-v* | gpg --clearsign > MD5SUMS
	cd build && sha1sum daptin-cli-v* | gpg --clearsign > SHA1SUMS
	cd build && sha256sum daptin-cli-v* | gpg --clearsign > SHA256SUMS
...
upload:
	rclone -P copy build/ memstore:downloads-daptin-cli-org/$(TAG)
...
beta:
	...
	rclone -v copy build/ memstore:cli.dapt.in/$(TAG)
	@echo Beta release ready at https://pub.daptin.in/$(TAG)/
...
winzip:
	zip -9 daptin-cli-$(TAG).zip daptin-cli.exe
